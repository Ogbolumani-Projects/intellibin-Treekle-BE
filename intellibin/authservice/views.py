...
class VerifyEmailView(APIView):
    ...
    def post(self, request, username):
        ...
        if serializer.is_valid():
            if user_otp and user_otp.otp_code == serializer.validated_data['otp_code']:
                if user_otp.otp_expires_at > timezone.now():
                    user.is_active = True
                    user.save()
                    return Response({"message": "Account activated successfully! You can login."}, status=status.HTTP_200_OK)
                else:
                    return Response({"error": "The OTP has expired, request a new OTP."}, status=status.HTTP_400_BAD_REQUEST)
            else:
...
class ResendOTPView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            otp = create_token(user)  # Assuming this function creates an OTP token
            ...
            """
            sender = "the.ayoadeborah@gmail.com"
            receiver = [user.email]
...
#         recipient_list = ['seminireayo@gmail.com']
